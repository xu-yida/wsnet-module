...
// <-RF00000000-AdamXu-2019/05/28-qlearning.
#define NUM_STATE (macMaxBE - macMinBE + 1)
// prev, keep, next
#define NUM_ACTION 3

#define RATE_QL_CSMA 0.6
#define DISCOUNT_QL_CSMA 0.9
// ->RF00000000-AdamXu
...
struct nodedata {
	...
	// <-RF00000000-AdamXu-2019/05/28-qlearning.
	// 0: prev; 1: keep; 2: next
	int q_action;
	int q_BE;
	double q[NUM_STATE][NUM_ACTION];
	// ->RF00000000-AdamXu
;
};
...
int setnode(call_t *c, void *params) {
	...
	nodedata->q_action = 1;
	nodedata->q_BE = 0;
	memset(nodedata->q, 0, sizeof(nodedata->q));
	// ->RF00000000-AdamXu
...
// ->RF00000000-AdamXu
	...
	// <-RF00000000-AdamXu-2019/05/28-qlearning.
	if(macMinBE <= nodedata->q_BE && macMaxBE >= nodedata->q_BE)
	{
		nodedata->BE = nodedata->q_BE - 1;
	}
	// ->RF00000000-AdamXu
        /* Backoff */
	...
	if(NUM_STATE <= state_order)
	{
		state_order = NUM_STATE-1;
	}
	// on the min border
	if(0 == state_order)
	{
		// keep
		MAX_BI(nodedata->q[state_order][1], nodedata->q[state_order+1][1], next_q);
		temp_q[0] = nodedata->q[state_order+next_q][1];
		
		// next
		MAX_TRI(nodedata->q[state_order][2], nodedata->q[state_order+1][2], nodedata->q[state_order+2][2], next_q);
		temp_q[1] = nodedata->q[state_order+next_q][2];
		
		// decide action
		MAX_BI(temp_q[0], temp_q[1], next_action);
		next_action = next_action + 1;
	}
	// on the max border
	else if(NUM_STATE == state_order+1)
	{
		// prev
		MAX_TRI(nodedata->q[state_order-2][0], nodedata->q[state_order-1][0], nodedata->q[state_order][0], next_q);
		temp_q[0] = nodedata->q[state_order-2+next_q][0];
		
		// keep
		MAX_BI(nodedata->q[state_order-1][1], nodedata->q[state_order][1], next_q);
		temp_q[1] = nodedata->q[state_order-1+next_q][1];
	...
	else
	{
		...
		if(1 == state_order)
		{
			MAX_BI(nodedata->q[state_order-1][0], nodedata->q[state_order][0], next_q);
			temp_q[0] = nodedata->q[state_order-1+next_q][0];
		}
		else
		{
			MAX_TRI(nodedata->q[state_order-2][0], nodedata->q[state_order-1][0], nodedata->q[state_order][0], next_q);
		...
		// keep
		MAX_TRI(nodedata->q[state_order-1][1], nodedata->q[state_order][1], nodedata->q[state_order+1][1], next_q);
		temp_q[1] = nodedata->q[state_order-1+next_q][1];
		
		// next
		if(NUM_STATE == state_order+2)
		{
			MAX_BI(nodedata->q[state_order][2], nodedata->q[state_order+1][2], next_q);
			temp_q[2] = nodedata->q[state_order+next_q][2];
		}
		else
		{
			MAX_TRI(nodedata->q[state_order][2], nodedata->q[state_order+1][2], nodedata->q[state_order+2][2], next_q);
		...
		// decide action
		MAX_TRI(temp_q[0], temp_q[1], temp_q[2], next_action);
	}
	nodedata->q_BE = nodedata->BE -1 + next_action;
	nodedata->q[state_order][nodedata->q_action] = RATE_QL_CSMA*(-1 + nodedata->q[state_order-1+next_action][next_action]) + DISCOUNT_QL_CSMA*nodedata->q[state_order][nodedata->q_action];
	// ->RF00000000-AdamXu
	
        /* Update backoff */
// <-RF00000000-AdamXu-2019/05/28-qlearning.
        //if ((++nodedata->BE) > macMaxBE) {
        nodedata->BE = nodedata->BE -1 + next_action;
        if ((nodedata->BE) > macMaxBE) {
// ->RF00000000-AdamXu
...
void rx(call_t *c, packet_t *packet) {
	...
	nodedata->q_BE = nodedata->BE -1 + next_action;
	nodedata->q[state_order][nodedata->q_action] = RATE_QL_CSMA*(s_rcw[state_order] + nodedata->q[state_order-1+next_action][next_action]) + DISCOUNT_QL_CSMA*nodedata->q[state_order][nodedata->q_action];
	// ->RF00000000-AdamXu
        /* Destroy txbuf */
        packet_dealloc(packet);
        packet_dealloc(nodedata->txbuf);
